...
public class MedicineController {
	...
	// http://localhost:8080/medicine-api/medicines/brand/{brandId}
	@PostMapping("/medicines/brand/{brandId}")
	Medicine addMedicine(@RequestBody Medicine medicine, @PathVariable("brandId") int brandId) {
		return medicineService.addMedicine(medicine, brandId);
	}

	// http://localhost:8080/medicine-api/medicines
	@PutMapping("/medicines/category/{categoryId}")
	void updateMedicine(@RequestBody Medicine medicine, @PathVariable("categoryId") int categoryId) {
		medicineService.updateMedicine(medicine, categoryId);
	}
	...
	ResponseEntity<List<Medicine>> getByMedicineNameAndMedicineType(@PathVariable("medicineName") String medicineName,
		...
		List<Medicine> medicineByNameAndType = medicineService.getByMedicineNameAndMedicineType(medicineName,
				medicineType);
		HttpHeaders headers = new HttpHeaders();
