...
public class BrandController {
	...
	// http://localhost:8080/brand-api/brand/brandname/Dart
	...
	// http://localhost:8080/brand-api/brand/brandname/Dart Tablet/medicinename/Dart
	@GetMapping("/brand/brandname/{brandName}/medicinename/{medicineName}")
	ResponseEntity<List<Brand>> getByBrandNameAndMedicineName(@PathVariable("brandName") String brandName,
			@PathVariable("medicineName") String medicineName) {

		List<Brand> brandByNameAndBrandName = brandService.getByBrandNameAndMedicineName(brandName, medicineName);
		HttpHeaders headers = new HttpHeaders();
		headers.add("desc", "Get all");
		return ResponseEntity.ok().headers(headers).body(brandByNameAndBrandName);
